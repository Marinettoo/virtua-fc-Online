...
<div x-data="{ open: false }" class="relative">
    ...
    <button
        ...
        class="w-full h-9 px-3 flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white text-sm font-medium text-slate-700 hover:border-slate-300 hover:bg-slate-50 transition-colors"
    >
    ...
    <div
        ...
        class="absolute left-0 right-0 mt-1 z-30 bg-white rounded-lg shadow-lg border border-slate-200 py-1 max-h-60 overflow-y-auto"
    >
        <template x-for="option in {{ $options }}" :key="'{{ $uid }}-' + option.value">
            ...
            >
                ...
                <svg x-show="{{ $model }} === option.value" x-cloak class="w-4 h-4 text-sky-500 shrink-0" fill="currentColor" viewBox="0 0 20 20">
